...
模擬設備：
  - 燈光開關   (GA 1/0/1 開關, 1/0/2 狀態)  DPT 1.001
  - 調光燈     (GA 1/0/3 開關, 1/0/4 亮度, 1/0/5 狀態)
  - 溫度感測器 (GA 2/0/1 值)               DPT 9.001
  - 窗簾控制   (GA 3/0/1 上下, 3/0/2 位置, 3/0/3 位置狀態)
...
class KNXTunnelServer:
    ...
    def broadcast_states(self):
        ...
        log.info("Broadcast states to %d client(s) | Temp=%.1f°C Light=%s Dimmer=%s(%d%%) Cover=%d%%",
                 len(self.connections),
                 s["temperature"],
                 "ON" if s["light_on"] else "OFF",
                 "ON" if s["dimmer_on"] else "OFF",
                 s["dimmer_brightness"],
                 s["cover_position"])
    ...
    async def run(self):
        ...
        log.info("  調光燈      GA 1/0/3(cmd) 1/0/4(bri) 1/0/5(state)")
        log.info("  溫度感測器  GA 2/0/1(value)             DPT 9.001")
        log.info("  窗簾        GA 3/0/1(updn) 3/0/2(pos) 3/0/3(state)")
        log.info("")
