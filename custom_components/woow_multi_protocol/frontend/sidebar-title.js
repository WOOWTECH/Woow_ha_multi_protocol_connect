/* Woow Multi-Protocol Connect — Sidebar Title i18n
 * Translates the single sidebar panel title to match HA's language setting.
...
/* Inject :not(:defined) CSS so the panel shows a spinner (not a blank screen)
 * while its JS bundle is still fetching. The panel lives inside
 * home-assistant > (shadow) > home-assistant-main > (shadow), so document-level
 * CSS cannot reach it — we inject into the correct shadow root.
 */
(function () {
  var CSS_ID = "woow-mp-panel-loading-css";
  var CSS_TEXT = [
    "woow-multi-protocol-panel:not(:defined) {",
    "  display: block;",
    "  min-height: 100vh;",
    "  background: var(--primary-background-color, #fafafa);",
    "}",
    "woow-multi-protocol-panel:not(:defined)::after {",
    "  content: '';",
    "  display: block;",
    "  width: 36px;",
    "  height: 36px;",
    ...
    "  border-radius: 50%;",
    "  animation: woow-mp-spin 0.8s linear infinite;",
    "}",
    "@keyframes woow-mp-spin { to { transform: rotate(360deg); } }"
  ].join("\n");
...
(function () {
  var PANEL_KEY = "woow_multi_protocol";
  var TITLES = {
    "en": "Woow Multi-Protocol Connect",
    "zh-Hant": "Woow 多協定連接"
  };
  ...
  function updateTitle(hass, main, title) {
    ...
    // New object references trigger a LitElement re-render.
  ...
  // Phase 2: Subscribe to core_config_updated (system-level language change)
