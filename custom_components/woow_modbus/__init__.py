"""Woow Modbus Setup Guide - Sidebar panel with built-in YAML editor."""
...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    """Set up the Woow Modbus component from yaml (unused, config_flow only)."""
    return True


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Woow Modbus Setup Guide from a config entry."""
    ...
    websocket_api.async_register_command(hass, handle_woow_modbus_ws)

    _LOGGER.info("Woow Modbus Setup Guide panel registered (config dir: %s/)", CONFIG_SUBDIR)
    return True
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.info("Woow Modbus Setup Guide panel removed")
    return True
...
async def handle_woow_modbus_ws(
